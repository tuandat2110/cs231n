...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        score_layer1, cache_layer1 = affine_forward(X, W1, b1)
        score_relu, cache_relu = relu_forward(score_layer1)

        scores, cache = affine_forward(score_relu, W2, b2)
        ...
        loss = -np.sum(np.log(softmax[range(N), y]))/N + 0.5*reg*(np.sum(W1*W1) + np.sum(W2*W2))

        softmax[range(N), y] -= 1
        dout_relu, grads['W2'], grads['b2'] = affine_backward(softmax, cache)
        grads['W2'] = grads['W2']/N + reg*W2
        grads['b2'] /= N

        dout_layer1 = relu_backward(dout_relu, cache_relu)
        _, grads['W1'], grads['b1'] = affine_backward(dout_layer1, cache_layer1)
